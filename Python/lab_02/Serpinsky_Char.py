...
n = (3 ** n) - 1 # каждые 3 ** n строчек в тр Паскаля = 1 уровень глубины в тр Серпинского
...
for i in range(1, n + 1):
    for j in range(1, i + 1):
        p_triangle[i][j] = (p_triangle[i - 1][j] + p_triangle[i - 1][j - 1]) % 3 # заполняем тр Паскаля 1, 2 и 0

for i in range(0, n + 1): # заполняем матрицу для тр Серпинского
    for j in range(0, n + 1):
        if p_triangle[i][j] == 1 or p_triangle[i][j] == 2: # любая единица или двойка в тр Паскаля = * в тр Серпинского
            s_triangle[i][j] = "*"
        else:
